...
export default function App() {
  ...
  async function syncAllToGcal(){
    if(!gcalToken){ showToast("יש לחבר Google Calendar קודם","error"); return; }
    setSyncing(true);
    let synced=0, failed=0;
    for(const shoot of shoots){
      if(!shoot.date||!shoot.clientName) continue;
      try{
        if(shoot.calendarEventId){
          await gcalUpdate({...shoot});
        } else {
          const calId=await gcalCreate(shoot);
          if(calId){
            await sbFetch(`shoots?id=eq.${shoot.id}`,{method:"PATCH",body:JSON.stringify({calendar_event_id:calId})},authToken);
            setShoots(prev=>prev.map(s=>s.id===shoot.id?{...s,calendarEventId:calId}:s));
          }
        }
        synced++;
      }catch{ failed++; }
    ...
    setSyncing(false);
    if(failed===0) showToast(`סונכרנו ${synced} אירועים ✓`);
    else showToast(`${synced} הצליחו, ${failed} נכשלו`,"error");
  ...
  return (
    <div style={S.root}>
      ...
      <div style={{...S.settingsPanel,transform:settingsOpen?"translateX(0)":"translateX(-100%)"}}>
        ...
        {gcalToken&&<div style={S.settingRow}>
          <div>
            <div style={S.settingLabel}>סנכרן הכל ל-Google Calendar</div>
            <div style={{fontSize:12,color:"#94a3b8",marginTop:2}}>מעדכן את כל האירועים</div>
          </div>
          <button style={{...S.settingBtnBlue,opacity:syncing?0.6:1}} onClick={syncAllToGcal} disabled={syncing}>
            {syncing?"...":"סנכרן"}
          </button>
        </div>}
        <div style={S.settingRow}>
