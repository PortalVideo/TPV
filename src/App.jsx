  ...
  return (
    <div style={S.root}>
      ...
      <nav style={S.bottomNav}>
        ...
        {[{v:"finances",icon:Icon.finance,label:"כספים"}].map(item=>(
          <button key={item.v} style={{...S.navItem,...(view===item.v?S.navItemActive:{})}} onClick={()=>setView(item.v)}>
            <span style={{color:view===item.v?"#1d4ed8":"#94a3b8",display:"flex"}}>{item.icon}</span>
            <span style={{...S.navLabel,color:view===item.v?"#1d4ed8":"#94a3b8"}}>{item.label}</span>
          </button>
...
const S = {
  ...
  main:{maxWidth:520,margin:"0 auto",padding:"16px 16px 24px"},
  ...
  // Bottom nav
  bottomNav:{position:"fixed",bottom:0,left:0,right:0,background:"rgba(255,255,255,0.9)",backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",borderTop:"1px solid rgba(219,234,254,0.6)",display:"flex",alignItems:"center",justifyContent:"space-around",padding:"8px 8px",paddingBottom:"calc(8px + env(safe-area-inset-bottom))",zIndex:40},
