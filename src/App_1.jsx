      ...
      <Modal open={modal==="new-event"} onClose={()=>{setModal(null);setEditId(null);setForm(initialForm);}} title={editId?"עריכת אירוע":"אירוע חדש"}>
        ...
        <FormGroup label="חבילה">
          <div style={{display:"flex",gap:10}}>
            {[{val:"חבילה ראשונה",num:"1"},{val:"חבילה שנייה",num:"2"}].map(p=>(
              <button key={p.val} style={{flex:1,padding:"11px 8px",borderRadius:12,fontSize:14,fontWeight:700,fontFamily:"inherit",cursor:"pointer",transition:"all 0.15s",
                background:form.package===p.val?"linear-gradient(135deg,#1d4ed8,#3b82f6)":"rgba(248,250,252,0.9)",
                color:form.package===p.val?"#fff":"#64748b",
                border:form.package===p.val?"1px solid #1d4ed8":"1px solid rgba(226,232,240,0.8)"}}
                onClick={()=>{
                  const isSecond = p.val==="חבילה שנייה";
                  setForm({...form, package:p.val, drone: isSecond?true:form.drone, vintage: isSecond?true:form.vintage});
                }}>
                חבילה {p.num}
              </button>
            ))}
